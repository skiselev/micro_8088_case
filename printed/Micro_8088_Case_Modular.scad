...
module case_back1() {
    vent_holes_top_height=inside_height-vent_hole_radius-6;
    vent_holes_start_width=(inside_width-18*vent_holes_horiz_step)/2-5;
    union() {     
        difference() {
            // case back side
            ...
            translate([0,case_thickness,case_thickness]) {
                // case back side
                // cutout for slots
                translate([-tolerance,slots_area_left,standoff_height+pcb_thickness]) {
                    cube([case_thickness+tolerance*2,slots_area_width,bracket_height-case_thickness]);
                }
                // cutout for DC jack
                translate([-tolerance,dc_jack_y,dc_jack_z]) {
                    rotate([0,90,0]) cylinder(h=case_thickness+tolerance*2,r1=10,r2=10-case_thickness);
                }
                // ventilation openings
                // top row
                for (vent_y = [vent_holes_start_width+vent_holes_horiz_step : vent_holes_horiz_step : inside_width-5]) {
                    translate([-tolerance,vent_y,vent_holes_top_height]) {
                        rotate([0,90,0]) cylinder(h=case_thickness+tolerance*2,r=vent_hole_radius,$fn=6);
                    }
                }

                for (vent_y = [vent_holes_start_width+vent_holes_horiz_step*17 : vent_holes_horiz_step : inside_width-5]) {
                    for (vent_z = [vent_holes_top_height-vent_holes_vert_step*2 : -vent_holes_vert_step*2 : dc_jack_z+15]) {
                        translate([-tolerance,vent_y,vent_z]) {
                            rotate([0,90,0]) cylinder(h=case_thickness+tolerance*2,r=vent_hole_radius,$fn=6);
                        }
                    }
                }
                for (vent_y = [vent_holes_start_width+vent_holes_horiz_step*16.5 : vent_holes_horiz_step : inside_width-5]) {
                    for (vent_z = [vent_holes_top_height-vent_holes_vert_step : -vent_holes_vert_step*2 : dc_jack_z+15 ]) {
                        translate([-tolerance,vent_y,vent_z]) {
                            rotate([0,90,0]) cylinder(h=case_thickness+tolerance*2,r=vent_hole_radius,$fn=6);
                        }
                    }
        ...
        translate([case_thickness,case_thickness,case_thickness]) {
            // DC jack mount
            translate([0,dc_jack_y,dc_jack_z]) rotate([0,90,0]) {
                difference() {
                    cylinder(h=bracket_thickness,r1=10+bracket_thickness,r2=10);
                    translate([0,0,-0.1]) cylinder(h=bracket_thickness+tolerance*2,r=4);
                }
            }
            // ISA slots
            difference() {
                union() {
                    // bottom part (under slots opennings)
                    translate([0,slots_area_left,standoff_height+pcb_thickness-case_thickness]) {
                        cube([bracket_depth,slots_area_width,case_thickness]);
                    }
                    // left side
                    translate([0,slots_area_left-bracket_thickness,standoff_height+pcb_thickness-case_thickness]){
                        cube([bracket_depth,bracket_thickness,case_thickness+bracket_height]);
                    }

                    // right side
                    translate([0,slots_area_left+slots_area_width,standoff_height+pcb_thickness-case_thickness]){
                        cube([bracket_depth,bracket_thickness,case_thickness+bracket_height]);
                    }
                    // middle (slot cutouts area)
                    translate([bracket_depth-bracket_thickness,slots_area_left,standoff_height+pcb_thickness]) {
                        cube([bracket_thickness,slots_area_width,bracket_height]);
                    }
                    // top/horizontal
                    translate([0,slots_area_left,standoff_height+pcb_thickness+bracket_height-case_thickness]) {
                        cube([bracket_depth,slots_area_width,case_thickness]);
                    }
                }
                translate([0,mb_to_case_left+mb_to_left_slot,standoff_height+pcb_thickness]){
                    // ISA slot cutouts
                    for(slot_num = [0 : slots-1]) {
                        translate([0,slot_spacing*slot_num,0]) {
                            card_slot1(slot_num);
                        }
                    }
                }
                // cutout to remove the ISA slots divider
                translate([bracket_depth-bracket_thickness-tolerance,slots_area_left-0.6-tolerance,standoff_height+pcb_thickness-case_thickness/2-tolerance]) {
                    cube([bracket_thickness+tolerance*2,slots_area_width+0.6*2+tolerance*2,bracket_height+tolerance*2]);
                }
                translate([bracket_depth-bracket_thickness-tolerance,slots_area_left-bracket_thickness-tolerance,20-tolerance]) {
                    cube([bracket_thickness+tolerance*2,slots_area_width+bracket_thickness*2+tolerance*2,bracket_height-20-tolerance*2]);
                }
                translate([bracket_depth-bracket_thickness-5,slots_area_left-bracket_thickness*1.5-tolerance,20-tolerance]) {
                    rotate([0,0,45])
                    cube([bracket_thickness/sqrt(2),bracket_thickness/sqrt(2),bracket_height-20-tolerance*2]);
                }
                translate([bracket_depth-bracket_thickness-5,slots_area_left+slots_area_width+bracket_thickness*0.5-tolerance,20-tolerance]) {
                    rotate([0,0,45])
                    cube([bracket_thickness/sqrt(2),bracket_thickness/sqrt(2),bracket_height-20-tolerance*2]);
                }
            }
...
module slots_cover1() {
    ...
    translate([case_thickness,case_thickness,case_thickness]) {
        difference() {
            union() {
                translate([bracket_depth-bracket_thickness,slots_area_left-0.6,standoff_height+pcb_thickness-case_thickness/2]) {
                    cube([bracket_thickness,slots_area_width+0.6*2,bracket_height]);
                }
                translate([bracket_depth-bracket_thickness,slots_area_left-bracket_thickness-tolerance,20]) {
                    cube([bracket_thickness,slots_area_width+bracket_thickness*2+tolerance*2,bracket_height-20]);
                }

                translate([bracket_depth-5-bracket_thickness*1.5,slots_area_left-bracket_thickness*2-tolerance,20]) {
                    cube([5+bracket_thickness*1.5,bracket_thickness,bracket_height-20]);
                }

                translate([bracket_depth-5-bracket_thickness*1.5,slots_area_left+slots_area_width+bracket_thickness+tolerance,20]) {
                    cube([5+bracket_thickness*1.5,bracket_thickness,bracket_height-20]);
                }


                translate([bracket_depth-bracket_thickness-5,slots_area_left-bracket_thickness*1.5-tolerance,20]) {
                    rotate([0,0,45])
                    cube([bracket_thickness/sqrt(2),bracket_thickness/sqrt(2),bracket_height-20]);
                }
                translate([bracket_depth-bracket_thickness-5,slots_area_left+slots_area_width+bracket_thickness*0.5+tolerance,20]) {
                    rotate([0,0,45])
                    cube([bracket_thickness/sqrt(2),bracket_thickness/sqrt(2),bracket_height-20]);
                }
            }
...
// The origin is:
// X - back edge of inside of the case
// Y - middle of the slot cut-out
// Z - top side of the motherboard
module card_slot1(slot_num=0) {
    bracket_top_offset=0.11*in2mm;
    //cutout_width=0.625*in2mm; // too wide, typical case has 0.5"
    cutout_width=0.5*in2mm;
//    cutout_height=3.688*in2mm;
    ...
    cutout_radius=1.27;
    mb_to_cutout=8.06-pcb_thickness;
    small_cutout_width=0.375*in2mm;
    small_cutout_height=bracket_height-cutout_height-mb_to_cutout;
    small_cutout_depth=0.32*in2mm;
    hole_x=0.25*in2mm;
    hole_y=-bracket_width/2+bracket_top_offset+0.74*in2mm-0.125*in2mm;
    // cutout - bigger part
    // Note: For some reason OpenSCAD has a rounding error here.
    //       Cut-out thickness increased to avoid this artifact.
    translate([bracket_depth-bracket_thickness-tolerance,-cutout_width/2+cutout_radius,mb_to_cutout+cutout_radius]) {
        minkowski() {
            cube([bracket_thickness+tolerance*2,cutout_width-cutout_radius*2,cutout_height-cutout_radius*2]);
            rotate([0,90,0]) cylinder(h=bracket_thickness+tolerance*2,r=cutout_radius);
        }
    ...
    // cutout - smaller part
    translate([bracket_depth-bracket_thickness-small_cutout_depth+bracket_thickness,-small_cutout_width/2,mb_to_cutout+cutout_height-tolerance]){
//        cube([small_cutout_depth+tolerance,small_cutout_width,small_cutout_height+tolerance*2]);
    }
    // mounting hole
    translate([hole_x,hole_y,bracket_height-case_thickness-tolerance]) {
         cylinder(h=case_thickness+tolerance*2,r=m3_tap_hole_radius);
    }
}

// The origin is:
// X - top side of the motherboard
// Y - middle of the slot cut-out
// Z - back edge of inside of the case
module card_slot(slot_num=0) {
//    cutout_width=0.625*in2mm; // too wide, typical case has 0.5"
    cutout_width=0.5*in2mm;
//    cutout_height=3.688*in2mm;
    cutout_height=95;
    ...
    // Note: For some reason OpenSCAD has a rounding error here.
    //       Cut-out thickness increased to avoid this artifact.
    translate([-cutout_height-mb_to_cutout-cutout_radius,-cutout_width/2+cutout_radius,bracket_depth-bracket_thickness-tolerance]) {
        minkowski() {
...
// size: X*Y*Z = 15*12*15
// hole centers: X*Y*Z = 9,9,0 and X*Y*Z = 0,9,9
module nut_inserts_corner1() {
    union() {
        translate([3,0,0]) {
            nut_insert1();
        }
